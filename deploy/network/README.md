...
- 七条 A 记录已在阿里云 DNS 建好（SRV 决定不配，玩家手动带端口即可）
...
直连没有 PROXY protocol 头，转发器对此是原生支持的：`resolveProxyHeader()` 判定对端没发头
时直接取 TCP 层的对端地址，而 OpenWrt 的 DNAT 保留源 IP，所以拿到的就是玩家真实地址。
线路归属靠"进的哪个入口端口"判定，与有没有 PROXY 头无关。
...
getter 一律回退到 `enabled = false`，转发器会静默地不启动。必须手工把 `[network]` 段
补进 common.toml。
...
- **十台机器（六台云节点 + 家里四台）已统一启用 BBR + fq**（2026-08-14）。配置写在
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `hangzhou1` | 杭州一线 | 阿里云ECS-杭州1 | 47.118.28.70 | ECS | 7000 | 25601 | 7401 | 开 |
| `shanghai` | 上海线 | 阿里云ECS-上海1 | 101.133.234.218 | ECS | 7000 | 25602 | 7402 | 开 |
...
| `xiamen` | 厦门家宽直连 | 家庭宽带（厦门联通 AS4837） | 175.44.1.151（动态，由 `home.shinoyuki.cn` 跟随） | 家宽 | 不经 frp | 25607 | 无 | **关** |
...
**但服务端在监听不等于外面进得来。** 2026-08-14 从 Minecraft 主机实测的结果：

| 节点 | `quicBindPort` | frps UDP 监听 | 实测 QUIC 登录 |
| --- | --- | --- | --- |
| ECS-深圳1 | 34566 | 有 | **成功**（45ms 内 login success） |
| ECS-杭州1 | 34566 | 有 | **超时** —— 安全组未放行 UDP |
| ECS-上海1 | 34566 | 有 | **超时** —— 安全组未放行 UDP |

阿里云安全组对 UDP 和 TCP 是分开放行的，开了 TCP 7000 不等于开了 UDP 34566。
所以**必须去阿里云控制台给杭州1 与上海的安全组添加入方向规则**：协议 UDP、
端口 34566、源 `0.0.0.0/0`（或收窄到家宽出口 IP）。深圳那台之前配 QUIC 时已经放行过，
这也是它唯一能通的原因。

**放行之前，这两条线的 frpc 必须继续用 TCP**，配成 quic 只会一直连不上。
...
`home` 关闭 PROXY protocol 的原因：直连线路玩家的 TCP 源地址本身就是真实地址，
...
延迟探针是另一条独立通路。六条 frp 线走各自节点终结 TLS，`home` 因为不经过任何云节点，
TLS 必须在家里那台机器上终结：
...
`frpc-template.toml` 派生的配置：`frpc@hz1`、`frpc@sz`、`frpc@wh`、`frpc@gz`、
`frpc@hz2`、`frpc@sh`。`home` 是直连线，不需要 frpc。

六个实例的 `webServer.port` 必须两两错开（7401-7407，见上方参数表），否则同机
...
| 文件 | 部署位置 | 作用 |
| --- | --- | --- |
| `frpc-template.toml` | 家里，派生成六份 | frpc 配置模板，按第一节参数表替换占位符 |
...
六条 mcwok.cn 线路都监听标准端口 25565，玩家只填域名、不带端口即可进服；
`home.shinoyuki.cn` 同样是 25565。

`home` 那条记录的 TTL 要填套餐允许的最小值。阿里云 DNS 免费版最低 600 秒，付费版可降到
60 秒。填不下去是套餐限制而非配置错误。TTL 越小，家宽换 IP 后玩家恢复得越快。

### SRV 记录（裸域名分流）

主机记录一律是 `_minecraft._tcp`，让玩家直接输 `mcwok.cn` 就能进服。
阿里云 DNS 的记录值格式是「优先级 权重 端口 目标」四段空格分隔，目标结尾的点不能省：

| 记录值 | 指向 | 档次 |
| --- | --- | --- |
| `1 10 25565 hangzhou1.mcwok.cn.` | 杭州一线 | ECS，优先级 1 |
| `1 10 25565 shanghai.mcwok.cn.` | 上海线 | ECS，优先级 1 |
| `1 10 25565 shenzhen.mcwok.cn.` | 深圳线 | ECS，优先级 1 |
| `2 10 25565 hangzhou2.mcwok.cn.` | 杭州二线 | 轻量，优先级 2 |
| `2 10 25565 wuhan.mcwok.cn.` | 武汉线 | 轻量，优先级 2 |
| `2 10 25565 guangzhou.mcwok.cn.` | 广州线 | 轻量，优先级 2 |

SRV 的优先级数字**越小越优先**，所以 ECS 填 1、轻量填 2。同优先级之间按权重分配，
这里全填 10 即等权。

`xiamen` 家宽线**刻意不放进 SRV**：它是动态 IP，DDNS 同步存在窗口期，一旦解析滞后
就会把裸域名的玩家整批送进黑洞。想走家宽的玩家直接填 `home.shinoyuki.cn` 即可。

关于 SRV 的实际效果要有正确预期：**它不是故障转移，也不是可靠的负载均衡。**
Minecraft 客户端解析 SRV 后只会拿到一个目标去连，连不上就是连不上，不会自动重试
下一条；同优先级多条记录的权重分配在各客户端实现里也不一致，很多实现直接取第一条。
所以 SRV 在这里的作用仅仅是「给裸域名一个默认去向」，真正的选路仍然靠玩家自己按
自查页面的实测结果挑子域名。
...
| 主机记录 | 类型 | 记录值 | 说明 |
| --- | --- | --- | --- |
| `_acme-challenge.*` | TXT | 由 ACME 客户端写入 | **不要手工创建**。仅家宽线走 DNS-01 时才会用到；六个云节点走 HTTP-01，不产生此记录 |

探针不需要额外的解析记录 —— `wss://<线路域名>/probe` 复用同一条 A 记录的 443 端口，
