...
### 传输协议：只有深圳支持 QUIC，且 frp 不会自动回退

实测（2026-08-14）六台节点的 UDP 监听情况：

| 节点 | `quicBindPort` | frps 的 UDP 监听 | 可用协议 |
| --- | --- | --- | --- |
| ECS-深圳1 | `34566` | `*:34566` | TCP + **QUIC** |
| 其余五台 | 未配置 | 无 | 仅 TCP |

QUIC 跑在 UDP 上，五台节点的 frps 根本没有 UDP 监听，所以给它们的 frpc 配
`transport.protocol = "quic"` 会直接连不上 —— 这不是调优问题，是压根没有服务端在听。
要让某台支持 QUIC，得先在它的 frps 配置里加 `quicBindPort` 并重启 frps。
...
因此本方案的选择是：**六条 frp 线主用 TCP，深圳那条保留现有的 QUIC。**

这不是折衷，而是把冗余放在了正确的层次上。七条线路本身就是容灾单元，不需要每条线
内部再做协议冗余；反过来，让深圳保持 QUIC 还额外买了一份保险 —— 万一某天运营商对
长连接 TCP 做针对性干扰，QUIC 这条走 UDP 的线仍然能用。玩家换条线即可，比在单条
线路内部实现协议切换简单得多也可靠得多。
