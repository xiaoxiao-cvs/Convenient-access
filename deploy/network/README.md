...
| `xiamen` | 厦门家宽直连 | 家庭宽带（厦门联通 AS4837） | 175.44.0.36（动态） | 家宽 | 不经 frp | 25607 | 无 | **关** |
...
| `xiamen` | A | 175.44.0.36 | 最低值 | 家宽直连，动态 IP，由 DDNS 脚本维护 |
| `@` | A | 120.24.184.115 | 600 | 裸域名兜底，指向深圳 |

七条线都监听标准端口 25565，玩家只填域名、不带端口即可进服。

`xiamen` 的 TTL 要填套餐允许的最小值。阿里云 DNS 免费版最低 600 秒，付费版可降到 60 秒。
填不下去是套餐限制而非配置错误。TTL 越小，家宽换 IP 后玩家恢复得越快。
...
就会把裸域名的玩家整批送进黑洞。想走家宽的玩家直接填 `xiamen.mcwok.cn` 即可。
